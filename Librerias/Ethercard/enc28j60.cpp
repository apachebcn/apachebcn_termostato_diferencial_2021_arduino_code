...
byte ENC28J60::initialize (uint16_t size, const byte* macaddr, byte csPin) {
    ...
    delay(2); // errata B7/2
    unsigned long timer_ = millis(); //INSERTADO POR APACHEBCN
    while (!readOp(ENC28J60_READ_CTRL_REG, ESTAT) & ESTAT_CLKRDY)
    { //INSERTADO POR APACHEBCN
        if (millis() - timer_ > 3000) return 0; //INSERTADO POR APACHEBCN
    } //INSERTADO POR APACHEBCN
    //    ;  //COMENTADO POR APACHEBCN
